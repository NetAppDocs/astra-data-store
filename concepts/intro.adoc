...
Astra Data Store is a Kubernetes native shared file service for both container and VM workloads delivering enterprise data management.

With Astra Data Store, you can do the following:

* *Move cloud-native applications*: Move an application and its data from one Kubernetes cluster to another, no matter where the clusters are located. ​
...
image:ads-intro.png[Astra Data Store Kubernetes-native storage and data management]
