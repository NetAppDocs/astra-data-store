...
|Deployment
|* Deployed on bare metal or virtual volumes.
...
* Deployed with the Kubernetes applications running in the same cluster on the same node pool (converged)

* Deployed with the Kubernetes applications running in the same cluster on different node pools (disaggregated)
