...
See link:../use/uninstall-ads.html#troubleshoot-the-astra-data-store-uninstall-process-2[Uninstall Astra Data Store preview manually without a script] or
link:../use/uninstall-ads.html#troubleshoot-the-astra-data-store-uninstall-process[Uninstall Astra Data Store preview automatically with a script].


=== Storage backend tasks not showing in vSphere
When an action to use an existing storage backend or remove a storag backend is submitted, the task does not appeaar in the Recent Tasks panel of vSphere client. 
