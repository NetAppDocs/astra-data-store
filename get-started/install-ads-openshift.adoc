...
To install Astra Data Store preview on Red Hat OpenShift Container Platform (OCP), download the installation bundle from the https://mysupport.netapp.com/site/products/all/details/astra-data-store/downloads-tab[NetApp Support Site^] and complete installation steps described in this procedure.
...
OpenShift SDN  (default CNI plugin for OpenShift) does not masquerade the traffic from the host loopback interface (127.0.0.1/localhost) to the HostPorts.

The Astra Data Store preview cluster requires the addition of a NAT rule for forwarding traffic from the localhost to the cluster service port (9920).

.Steps

. Note the line number for the current `KUBE-HOSTPORTS` rule in the OUTPUT chain for the NAT table.
+
In the example below `KUBE-HOSTPORTS` is at position 4.
+
----
...
. Add a new rule to the NAT table, right above the `KUBE-HOSTPORTS` entry.
+
----
$ sudo iptables -t nat -I OUTPUT 4 -s 127.0.0.1 -j KUBE-MARK-MASQ -p tcp --dport 9920
----

. Verify that the newly added rule is added to the NAT table just above the `KUBE-HOSTPORTS` rule.
+
----
$ sudo iptables -t nat -L OUTPUT --line-numbers
Chain OUTPUT (policy ACCEPT)
