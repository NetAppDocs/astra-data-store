...
OpenShift SDN  (default CNI plugin for OpenShift) doesn't masquerade the traffic from the host loopback interface (127.0.0.1/localhost) to the HostPorts.
...
+
----
sudo iptables -t nat -L KUBE-HOSTPORTS
...
+
----
$ sudo iptables -t nat -L KUBE-HP-6AYWWGPCDSHS2K25
...
+
----
$ sudo iptables -t nat -I KUBE-HP-6AYWWGPCDSHS2K25 2 -s 127.0.0.1 -j KUBE-MARK-MASQ
Chain KUBE-HP-6AYWWGPCDSHS2K25 (1 references)
