...
.What you'll need
* Before you begin installation, link:requirements.html[prepare your environment for Astra Data Store deployment].
...
* A link:requirements.html#licensing[NetApp license file] (NLF) for Astra Data Store preview. Instructions to download the license will be sent to you after you sign up.
* An active kubeconfig with cluster admin rights for the active context.
* An understanding of the link:faq-ads.html[roles and privileges] used by Astra Data Store preview.
...
. Log in to the https://mysupport.netapp.com/site/[NetApp Support Site] and download the Astra Data Store preview bundle (`2021.12_astradatastore.tar`).
...
== Create a custom SCC
//ocp
OpenShift uses security context constraints (SCC) that control the actions that a pod can perform and what it has the ability to access.

By default, the execution of any container will be granted the restricted SCC and only the capabilities defined by that SCC. Restricted SCC does not provide permissions required by Astra Data Store preview pods. Use this procedure to provide the required privileges (listed in the sample) to Astra Data Store preview.
...
+
----
[root@api-int ~]# oc get scc/ads-privileged
...
. Create `/var/lib/kubelet/config.yaml` file on each of the worker nodes before initiating the cluster installation.
+
----
sudo cp /var/lib/kubelet/config.json /var/lib/kubelet/config.yaml`
----
+
. Complete this procedure on all Kubernetes nodes before the cluster yaml is applied.

NOTE: If you do not do this, the Astra Data Store preview cluster installation will fail.
...
. Deploy the operator by using the `kubectl apply` command:
+
----
...
. Deploy by using the `kubectl apply` command:
+
----
...
. If SELinux is enabled, re-label the `selinux` context for the following directories on the nodes in the Astra Data Store preview cluster.
...
OpenShift SDN  (default CNI plugin for OpenShift) doesn't masqeurade the traffic from the host loopback interface (127.0.0.1/localhost) to the HostPorts.

The Astra Data Store preview cluster requires addition of a NAT rule for forwarding traffic from localhost to cluster service port (9920).

.Steps

. Note the chain used for hostport 9920.
+
----
[core@astraiop-ocp-c1w4 ssl]$ sudo iptables -t nat -L KUBE-HOSTPORTS
Chain KUBE-HOSTPORTS (2 references)
target prot opt source destination
KUBE-HP-6AYWWGPCDSHS2K25 tcp -- anywhere anywhere /* k8s_astrads-ds-ocp-cluster-stg8k_astrads-system_6ae5a9fb-1942-4a2d-9901-c27658925218_0_ hostport 9920 */tcp dpt:9920
----


. Find the rules within the chain and the position to insert the new rule. The new rule should be inserted  after the existing `KUBE-MARK-MASQ` rule.
+
In the below example, `KUBE-MARK-MASQ` is the first entry. You should insert the new rule at position 2.
+
----
[core@astraiop-ocp-c1w4 ssl]$ sudo iptables -t nat -L KUBE-HP-6AYWWGPCDSHS2K25
Chain KUBE-HP-6AYWWGPCDSHS2K25 (1 references)
target prot opt source destination
KUBE-MARK-MASQ all -- 10.128.4.2 anywhere /* k8s_astrads-ds-ocp-cluster-stg8k_astrads-system_6ae5a9fb-1942-4a2d-9901-c27658925218_0_ hostport 9920 */
DNAT tcp -- anywhere anywhere /* k8s_astrads-ds-ocp-cluster-stg8k_astrads-system_6ae5a9fb-1942-4a2d-9901-c27658925218_0_ hostport 9920 */ tcp to:10.128.4.2:9920
----

. Add a new rule to the chain at position noted in the above step.
+
----
[core@astraiop-ocp-c1w4 ssl]$ sudo iptables -t nat -I KUBE-HP-6AYWWGPCDSHS2K25 2 -s 127.0.0.1 -j KUBE-MARK-MASQ
Chain KUBE-HP-6AYWWGPCDSHS2K25 (1 references)
[core@astraiop-ocp-c1w4 ssl]$
----

. Verify the newly added rule is immediately below the existing the `KUBE-MARK-MASQ` rule.
+
----
[core@astraiop-ocp-c1w4 ssl]$ sudo iptables -t nat -L KUBE-HP-6AYWWGPCDSHS2K25
Chain KUBE-HP-6AYWWGPCDSHS2K25 (1 references)
target prot opt source destination
KUBE-MARK-MASQ all -- 10.128.4.2 anywhere /* k8s_astrads-ds-ocp-cluster-stg8k_astrads-system_6ae5a9fb-1942-4a2d-9901-c27658925218_0_ hostport 9920 */
KUBE-MARK-MASQ all -- localhost anywhere
DNAT tcp -- anywhere anywhere /* k8s_astrads-ds-ocp-cluster-stg8k_astrads-system_6ae5a9fb-1942-4a2d-9901-c27658925218_0_ hostport 9920 */ tcp to:10.128.4.2:9920
----

. Persist the rule across pod restarts.
+
----
[core@astraiop-ocp-c1w4 ssl]$ sudo iptables -t nat -I OUTPUT 4 -s 127.0.0.1 -j KUBE-MARK-MASQ -p tcp --dport 9920
----

NOTE: Perform the steps on all the nodes in the Astra Data Store preview cluster.

NOTE: Repeat  these steps if the Astra Data Store preview cluster node restarts.
