...
Astra Trident will not be uninstalled from the cluster if you uninstall Astra Data Store. Astra Trident is a CSI driver, managed separately from Astra Data Store and if required has to be uninstalled separately.
...
Astra Data Store preview is not supported on IPv6.
