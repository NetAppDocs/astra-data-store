...
Prepare the worker nodes for Astra Data Store preview cluster deployment. Perform this procedure on all the worker nodes used by the Astra Data Store cluster.

OpenShift uses a json format for the kubelet config file (`/var/lib/kubelet/config.json`). The Astra Data Store cluster looks for the yaml format of the `kubelet config` file.
...
NOTE: If you do not do this, the Astra Data Store cluster installation will fail.
