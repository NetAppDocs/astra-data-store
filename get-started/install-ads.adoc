...
. Edit the Astra Data Store operator custom resource defintion (CRD) yaml (`astrads_v1alpha1_astradsoperator_external.yaml`) to refer to your local registry and secret.
+
----
...
. Edit the Astra Data Store custom resource (CR) file (`astra_control_center_min.yaml`):
+
----
...
+
NOTE: Properties configured by the CR cannot be changed after initial Astra Data Store deployment.
