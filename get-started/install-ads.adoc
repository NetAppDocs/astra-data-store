...
.Reserve resources for data-plane components using kubelet config.json
//ocp
...
IMPORTANT: Any manual edits to this config file will leave your cluster in a degraded state. The procedure that follows updates this file using Openshift native options.

This procedure applies to the profile role "master" that is applied to all nodes and nodes labeled "worker". Some steps require updates for both roles.
...
. In the config for "master" and again for the config for "worker", add the `systemReserved` parameter to the copied Kubelet config:
+
NOTE: Use the same CPU and memory values you will use in your link:../get-started/install-ads.html#install-the-astra-data-store-cluster[Astra Data Store cluster CR]. If your config.json file was empty originally, add the `systemReserved` contents to it.

+
Example:
----
{"auths":{"docker.repo.eng.netapp.com":{"auth":"<auth token>"}}, "systemReserved":{"cpu":9,"memory":34}}
----

. In the config for "master" and again for the config for "worker", update the `contents.source` section of the template from link:substep_contents_source_data[step 1]. Use data from the `systemReserved` parameter you updated in the previous step in base64 format.

+
----
base64 {"auths":{"docker.repo.eng.netapp.com":{"auth":"<auth token>"}}, "systemReserved":{"cpu":9,"memory":34}}
...
. Apply both MachineConfig templates on your cluster:
+
----
