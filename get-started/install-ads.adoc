...
. Copy the kubectl-astrads binary from the directory you extracted images to the standard path where k8s kubectl binaries are installed; for example, `/usr/bin/`. kubectl-astrads is a custom kubectl extension that installs and manages Astra Data Store clusters.
+
----
...
+
----
sed -i 's/\[your_registry_path\]/docker.repo.example.com/' astradsoperator.yaml
----
...
== Apply the Astra Data Store version YAML
. Edit the Astra Data Store version custom resource (CR) file to refer to the local registry (`your_registry_path`) you pushed the images to in a <<substep_image_local_registry_push,previous step>>:
+
----
sed -i 's/\[your_registry_path\]/docker.repo.example.com/' astradsversion.yaml
----
