...
* In `adsDataNetworks`, add a comma-separated list of floating IP addresses (`addresses`) that are routable from any host where you intend to mount a NetApp volume.
...
----
+
During cluster deployment, the operation status should change from `blank` to `pending` to `created`. To monitor cluser events during depoyment, you can run the following command:
+
----
