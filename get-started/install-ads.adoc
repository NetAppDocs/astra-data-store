...
To install Astra Data Store preview either on a Kubernetes cluster or on the Red Hat OpenShift Container Platform (OCP), download the installation bundle from the https://mysupport.netapp.com/site/products/all/details/astra-data-store/downloads-tab[NetApp Support Site^] and complete installation steps described in this procedure.
...
* <<Create a namespace to deploy ADS preview (OCP only)>>
* <<Create a custom SCC (OCP only)>>
* <<Create the roles and role bindings (OCP only)>>
* <<Prepare the worker nodes (OCP only)>>
...
== Create a namespace to deploy Astra Data Store preview (OCP only)
