...
* A link:../get-started/requirements.html#licensing[NetApp license file (NLF)] for Astra Data Store preview.
* An active kubeconfig with cluster admin rights for the active context.
* An understanding of link:../get-started/faq-ads.html#installation-and-use-of-astra-data-store-preview-on-a-kubernetes-cluster[the roles and privileges] used by Astra Data Store.
...
== Download the Astra Data Store bundle and extract the images
. Log in to the https://mysupport.netapp.com/site/downloads/evaluation/astra-data-store/download[NetApp Support Site^] and download the Astra Data Store bundle (`2021.12_astradatastore.tar`).
...
* In `adsDataNetworks`, add a comma-separated list of floating IP addresses (`addresses`) that are routable from any host where you intend to mount a NetApp volume. Use one floating IP address per node.
...
+
NOTE: Cluster and storage networks need to be on the same interface.
...
----
+
During cluster deployment, the operation status should change from `blank` to `in progress` to `created`. Cluster deployment will last approximately 8 to 10 minutes. To monitor cluster events during deployment, you can run the following command:
+
----
kubectl get events --field-selector involvedObject.kind=AstraDSCluster -n astrads-system
----
+
You can also run `kubectl describe astradscluster <cluster name> -n astrads-system` to show events describing the process or to check operator logs if the deployment status is not changing.
