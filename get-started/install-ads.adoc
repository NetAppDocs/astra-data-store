...
* Remove the commenting around the `adsNodeSelector` section. Add a node label to identify the pool of worker nodes that support this Astra Data Store cluster.
+
...
//Node labels must be created or deployment will fail per ACC
* Specify the number of nodes that should be used in the Astra Data Store cluster (between 4-16).
 ...
 *# monitoringConfig:*
  ...
  autoSupportConfig:
    ...
    caseCreationURL: "https://m2m-stage.netapp.com"
