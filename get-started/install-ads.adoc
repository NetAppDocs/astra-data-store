...
You can install a standalone Astra Data Store for use in a standard Kubernetes environment, deploy it using Astra Control Center if you intend to use Astra Data Store with VMware or manage it using Astra Control, or install Astra Data Store for use in another supported Kubernetes distribution.
...
[%collapsible]
====
To install a standalone Astra Data Store, complete the installation steps described in <<Install an Astra Data Store,this procedure>>.
====
// End snippet

=== With Astra Control Center
// Start snippet: collapsible block (open on page load)
.Details
[%collapsible]
====
To use Astra Data Store with Astra Control Center, complete the installation steps described in <<Install Astra Data Store using Astra Control Center,this procedure>>.
====
// End snippet
...
[%collapsible]
====
To install Astra Data Store with Red Hat OpenShift Container Platform (OCP) environments, complete the installation steps described in <<Install an Astra Data Store,this procedure>>. Additional installation steps for Red Hat OpenShift Container Platform (OCP) environments are also described.
====
// End snippet
...
[%collapsible]
====
To install Astra Data Store with Rancher environments, complete the installation steps described in <<Install an Astra Data Store,this procedure>>, substituting Rancher commands for kubectl commands.
====
// End snippet
...
[%collapsible]
====
To install Astra Data Store with Anthos, complete the installation steps described in <<Install an Astra Data Store,this procedure>> and add the Astra Data Store cluster to your Anthos environment.

NOTE: For Anthos, Astra Data Store requires portmap chaining to be configured. You can enable portmap chaining mode using a Helm Cilium install by following the instructions in https://docs.cilium.io/en/v1.9/gettingstarted/cni-chaining-portmap/[Cilium documentation]. You can alternatively add `cni-chaining-mode: portmap` to the `cilium-config` configMap and restart Cilium pods to enable portmap chaining.
...
* Access to the https://mysupport.netapp.com/site/products/all/details/astra-data-store/downloads-tab[NetApp Support Site^]. https://www.netapp.com/cloud-services/astra/data-store-form/[Register^] for an evaluation if you don’t already have a full-access NetApp Support Site account.
...
. Verify the binary version is 22.5.0-X:
+
----
...
e100000006  astrads-example-cluster    true    Astra Data Store    true        2023-01-23  2022-04-04T14:38:54Z
----
...
* (Optional) Specify a specific number of nodes that should be used in the Astra Data Store cluster (between 4-16).
...
+
NOTE: For brevity, many comments have been removed from the YAML sample below.
...
astrads-example-cluster     created   2022.05.0  e100000006       10.x.x.x   13m
----
...
* You have installed Astra Control Center 22.04 or later.
...
* *Standalone and third-party distributions*: Complete the Astra Data Store deployment by performing additional link:setup-ads.html[setup tasks].
