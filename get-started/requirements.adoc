...
Astra Data Store preview needs a certain network interface and IP address configuration for the management, cluster, and storage networks. You can use several configurations, depending on the need for one (combined management and cluster traffic) or two (separate management and cluster traffic) network interfaces per node. No VLAN tags are used in these configurations.

NOTE: You should omit the data network gateway for each of these configurations. The existing routing configuration on each node should accommodate all of the addresses.
...
NOTE: This configuration is best for redundancy when using physical nodes.
|===
