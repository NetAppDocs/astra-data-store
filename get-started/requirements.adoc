...
* <<Astra Trident>>
* <<Container Network Interface requirements>>
* <<Licensing>>
...
Astra Data Store has been validated with the following Container Network Interfaces (CNIs). Note that if you deploy with a CNI, you need to disable the host firewall (firewalld).
...
NOTE: Astra Data Store deployed with the Cilium CNI requires portmap chaining. You will need to use the port-mapping plugin for HostPort support. You can enable CNI chaining mode by adding `cni-chaining-mode: portmap` to the cilium-config configMap and restarting the Cilium pods.
