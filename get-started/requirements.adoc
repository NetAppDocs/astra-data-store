...
== Astra Trident
Astra Data Store requires the application Kubernetes clusters to be running Astra Trident 22.01. Astra Data Store can be configured as a link:../get-started/setup-ads.html#set-up-astra-data-store-as-storage-backend[storage backend] with Astra Trident to provision persistent volumes.
