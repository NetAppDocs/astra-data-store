...
Astra Data Store preview needs a certain network interface and IP address configuration for the management, cluster, and storage networks.
...
You can configure Astra Data Store preview networking in several ways, depending on if you need to use one (combined management and cluster traffic) or two (separate management and cluster traffic) network interfaces. No VLAN tags are used in any of these configurations.
