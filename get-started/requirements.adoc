...
Astra Data Store preview needs a certain network interface and IP address configuration for the management, cluster, and storage networks. You can use several configurations, depending on the need for one or two network interfaces per node.

Each network requires the following bandwidth:

* Management network: 1GbE or faster interface
* Cluster network: 10GbE or faster interface
* Data network: 10GbE or faster interface
