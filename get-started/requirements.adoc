...
NOTE: Astra Data Store requires Astra Trident version 21.10.01 or 22.01 for storage provisioning and orchestration. See the link:setup-ads.html#install-astra-trident[Astra Trident installation instructions].
...
== Astra Trident
Astra Data Store requires the application Kubernetes clusters to be running Astra Trident 21.10.01 or 22.01. Astra Data Store can be configured as a link:../get-started/setup-ads.html#set-up-astra-data-store-as-storage-backend[storage backend] with Astra Trident to provision persistent volumes.
