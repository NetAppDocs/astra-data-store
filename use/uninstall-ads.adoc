...
= Uninstall Astra Data Store preview
:hardbreaks:
:icons: font
:imagesdir: ../media/use/
...
NOTE: The uninstaller takes about a minute to finish. Then, the removal of the Astra Data Store preview cluster-scoped resources starts.

=== Delete the license

. ssh to each worker node from the cluster and validate that `firetap` or `netwd` are not running in the worker nodes.
...
The following shows a sample of a manual uninstallation script.
