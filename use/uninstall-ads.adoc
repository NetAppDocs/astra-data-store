...
To uninstall Astra Data Store, you will remove workloads, bindings, volumes, volume snapshots, export policies, clusters, license, deployment environment, and the Astra Data Store namespace.
...
Deleting the cluster also uninstalls FireTap from the nodes, which will stop the `firetap` and `netwd` services.
