...
.Steps
.	If the new node’s dataIP is not already part of the cluster CR, do the following:
.. Edit the astradscluster CR and add the additional dataIP in the `adsDataNetworks` *addresses* field. Replace the information in capital letters with the appropriate values for your environment:
+
[source,sh]
----
