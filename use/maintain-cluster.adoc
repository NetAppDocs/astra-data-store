...
No suitable drives to add exist.
----
. Add the available drives to the respective nodes. In this example, replace the information in capital letters with the appropriate values for your environment:
+
----
kubectl astrads drives adddrive create --cluster=CLUSTER_NAME --name REQUEST_NAME --all
----

.Result
Astra Data Store creates a request to add all available drives, and a message appears with the result of the request.
...
// List all available drives to add
// Add the drives
