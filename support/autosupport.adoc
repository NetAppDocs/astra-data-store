...
== Upload a core file
If a service crashes, an ASUP message is created along with a core dump file. Astra Data Store preview automatically uploads the ASUP message to NetApp Support, but you need to manually upload the core dump file so that it is associated with the ASUP message.
...
----
. Use the following `kubectl` commands to download the core dump file from the ASUP message. Use the `-o` option to specify a destination directory for the downloaded file.
+
----
