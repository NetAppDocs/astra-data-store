...
+
NOTE: For the Astra Data Store Early Access Program (EAP) release, Astra Control Center and NetApp Astra Plugin for VMware vSphere should be deployed only on the `pcloud` namespace and not on a custom namespace.

* Astra Data Store is licensed and deployed. See link:../get-started/install-ads.html[Install Astra Data Store].

NOTE: For the Astra Data Store Early Access Program (EAP) release, Astra Control Center and Astra Data Store must be deployed in two different Kubernetes clusters.
