...
== Obtain VM storage policies

After you create a datastore and before you create VMs, you should retrieve VM storage policies by using the API ``/virtualization/api/v1/vcenters/vm-storage-policies`.

.Steps

. Access the Swagger page by going to `https://<ads_gateway_ip>:8443`[Swagger page].
. In Swagger, go to the API `POST /virtualization/api/auth/login` and provide the username, password and vcenter hostname.
+
Response:
----
{
...
. In Swagger, go to `GET /virtualization/api/auth/validate-session` and complete the following steps:
...
. In Swagger, go to ``/virtualization/api/v1/vcenters/vm-storage-policies` and add the bearer token generated in previous response as 'authorization'.
...
== What's next
Next, you might want to mount the datastore. See link:../use-vmware/manage-ads-vmware.html#mount-a-datastore[Mount a datastore].
